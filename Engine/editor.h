...
void editor_render_borders(void);
void editor_update(void);
void editor_ui(void);
void editor_open_wall_window(i32 sector_id, i32 quad_idx);

void editor_add_quad(level_sector_data_t* sector, const level_quad_t* template_quad);
...
static level_quad_t get_default_quad(camera_t* cam)
{
    return (level_quad_t){
        .pos = {
            roundf(cam->pos.x + cam->front.x * 3.0f),
            roundf(cam->pos.y + cam->front.y * 3.0f),
            roundf(cam->pos.z + cam->front.z * 3.0f)
        },
        .rot = {0, 0, 0},
        .size = {2, 2},
        .tex_id = 0,
