...
void mat4_ortho(f32* m, const f32 left, const f32 right, const f32 bottom,
...
bool mat4_inverse(const f32* m, f32* out)
{
    ...
    const f32 det = m[0]*tmp[0] + m[1]*tmp[4] + m[2]*tmp[8] + m[3]*tmp[12];
...
vec3s vec3_normalize(const vec3s a) { const f32 m=vec3_magnitude(a); return m==0 ? a : vec3_scale(a,1.0f/m); }
vec3s vec3_cross(const vec3s a, const vec3s b) { return (vec3s){a.y*b.z-a.z*b.y,a.z*b.x-a.x*b.z,a.x*b.y-a.y*b.x}; }
